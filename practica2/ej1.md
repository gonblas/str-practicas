...
2. Cada período divisible por $f$ o $(2f - \text{MCD}(T_i, f)) \le D_i$  
...
- Cumple:  
  - $f \ge \max(C_i) = 2$  
  - $2f - \text{MCD}(T_i, f) \le D_i$ → OK  
    - D: $2f - \text{MCD}(3,2) = 4-1 = 3 \le 3$  
    - C: $2f - \text{MCD}(6,2) = 4-2 = 2 \le 6$  
    - A: $2f - \text{MCD}(12,2) = 4-2 = 2 \le 12$  
    - B: $2f - \text{MCD}(24,2) = 4-2 = 2 \le 24$  
  - Hiperperíodo dividido en frames: $24 / 2 = 12$ frames (es entero)

Para crear la tabla de ejecución del ejecutivo cíclico dividimos el hiperperíodo en **frames de 2 unidades** y asignamos las tareas respetando su **período y tiempo de ejecución**:
...
Dado que el sistema es planificable en la duración de un hiperperíodo, lo será también durante toda la vida del sistema, ya que esta planificación se repite infinitamente.
